...
-- Intervention infos (si plusieurs lignes, on peut adapter pour prendre la plus récente si besoin)
LEFT JOIN intervention_infos inf ON inf.intervention_id = i.id

-- Jointure avec devis_generators : on s'assure de ne joindre qu'un seul enregistrement par devis (le plus récent)
LEFT JOIN (
    SELECT dg1.devis_id, dg1.site, dg1.code_site, dg1.created_at, dg1.updated_at
    FROM devis_generators dg1
    JOIN (
        SELECT devis_id, MAX(id) AS max_id
        FROM devis_generators
        GROUP BY devis_id
    ) dg2 ON dg1.devis_id = dg2.devis_id AND dg1.id = dg2.max_id
) AS dg ON dg.devis_id = i.devis_id

-- Gestion du générateur (priorité à champs de l'intervention)
...
WHERE i.type_service = 0 OR i.type_service = 2;
        ");
    }
