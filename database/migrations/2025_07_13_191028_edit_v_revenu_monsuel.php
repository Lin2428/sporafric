...
SELECT
    ...
    (
    SELECT IFNULL(SUM(cg.forfait), 0)
    FROM contracts c2
    JOIN contract_generators cg ON cg.contract_id = c2.id
    WHERE mois.mois BETWEEN DATE_FORMAT(c2.start_date, '%Y-%m') AND DATE_FORMAT(c2.end_date, '%Y-%m')
      AND mois.mois < DATE_FORMAT(CURRENT_DATE, '%Y-%m')
		) AS revenu_contract,

    -- Total des interventions (via infos → devis_montant)
    (
        SELECT IFNULL(SUM(i.montant), 0)
