                ...
                @if ($data->isNotEmpty())
                    @foreach ($data as $item)
                        <tr class="border-b border-slate-400">
                            <td class="px-3 py-2 text-blue-600 text-sm text-left border border-slate-400">
                            ...
                            <td class="px-3 py-2 text-[12px] text-left text-slate-800 border border-slate-400">
                                {{ $item->contractGenerator?->code_site ?? $item->devisGenerator?->devis?->code_site }}</td>
                                 <td class="px-3 py-2 text-sm text-left text-slate-800 border border-slate-400">
                                    {{ $item->name }}
                             </td>
                            <td class="px-3 py-2 text-sm text-right text-slate-800 border border-slate-400">
                                {{$item->power }}
                            </td>
                            <td class="px-3 py-2 text-sm text-right text-slate-800 border border-slate-400">
                                {{$item->houres }}
